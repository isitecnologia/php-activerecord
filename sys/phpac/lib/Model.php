...
class Model
{
	...
	public function __construct(array $attributes=array(), $guard_attributes=true, $instantiating_via_find=false, $new_record=true)
	{
		...
		$this->set_attributes_via_mass_assignment($attributes, $guard_attributes, false);
	 ...
	 */
	public function set_attributes(array $attributes, $throws_exception=true)
	{
		$this->set_attributes_via_mass_assignment($attributes, true, $throws_exception);
	}
	 ...
	 */
	private function set_attributes_via_mass_assignment(array &$attributes, $guard_attributes, $throws_exception=true)
	{
			...
			if ($guard_attributes)
			{
				...
				if ($use_attr_protected && in_array($name,static::$attr_protected))
					continue;
                                ...
                                if (in_array($name, $this->table()->pk))
                                        continue;
		...
		if ($throws_exception && !empty($exceptions))
  ...
  public function to_csv(array $options=array())
  {
	...
	public static function transaction($closure)
	{
        ...
        public function last_insert_id(){
                $id = $this->find_by_sql("SELECT @@IDENTITY as id");
                return $id[0]->id;
        }
